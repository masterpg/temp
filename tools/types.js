...
function buildTypes() {
  ...
  // ビルドを実行
  if (shell.exec(`${BUILD_TYPES}`).code !== 0) {
    console.log('ERROR'.bgRed, 'Build types failed'.red)
    shell.exit(1)
  }

  // "src/components"配下の".d.ts"ファイル一覧を取得
  const dtsFilePaths = glob.sync(`${path.join(SRC_COMPONENTS_DIR, '**/types.comp.d.ts')}`, { nodir: true })
